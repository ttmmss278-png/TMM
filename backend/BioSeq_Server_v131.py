...
from wgs_runner_v131 import detect_tool_backend, get_tool_versions, run_wgs, wsl_tools_available
    ...
    return jsonify(
        {
            ...
            "build": ENGINE_BUILD,
            "capabilities": ["wgs-native", "wgs-wsl", "reference-cache", "environment-check"],
        ...
        if not environment_state["ready"]:
            return (
                jsonify(
                    {
                        "status": "failed",
                        "message": "WGS toolchain is not ready. Run the current BioSeq Engine Manager.",
