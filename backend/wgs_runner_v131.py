...
def run_wsl_command(command, log):
    return run_process(wsl_prefix() + command, log)
...
def get_tool_versions(backend):
    ...
    commands = {
        "fastp": ["fastp", "--version"],
        "bwa": ["bash", "-lc", "bwa 2>&1 | head -n 3"] if backend == "wsl" else ["bwa"],
        "samtools": ["bash", "-lc", "samtools --version | head -n 1"] if backend == "wsl" else ["samtools", "--version"],
        "bcftools": ["bash", "-lc", "bcftools --version | head -n 1"] if backend == "wsl" else ["bcftools", "--version"],
    }
    versions = {}
...
def run_wgs(r1, r2, reference, output):
    ...
    if not backend:
        return {
            ...
            "log": (
                "Missing WGS tools: "
                + ", ".join(native_missing)
                + ". Run the current BioSeq Engine Manager to install or repair the WSL toolchain."
            ),
    ...
    try:
        ...
        if backend == "wsl":
            ...
            flagstat = run_wsl_command(["samtools", "flagstat", tool_bam], log)
        else:
            run_native_pipe(bwa_command, samtools_sort, log)
            run_process(["samtools", "index", tool_bam], log)
            flagstat = run_process(["samtools", "flagstat", tool_bam], log)
        (output_dir / "flagstat.txt").write_text(flagstat.stdout or "", encoding="utf-8")

        bcftools_available = (
            shutil.which("bcftools") is not None if backend == "native" else wsl_tools_available(["bcftools"])
        )
