...
from wgs_runner_v140 import (
    detect_tool_backend,
    get_tool_versions,
    portable_bundle_status,
    run_wgs,
    wsl_tools_available,
)
...
ENGINE_BUILD = "20260730-portable1"
...
def wgs_environment_status():
    ...
    elif backend == "portable-wsl":
        bcftools_available = wsl_tools_available(["bcftools"], portable=True)
    elif backend == "wsl":
        bcftools_available = wsl_tools_available(["bcftools"], portable=False)
    else:
        bcftools_available = False

    portable = portable_bundle_status()
    return {
        "ready": bool(backend),
        "backend": backend or "unavailable",
        "missing_tools": missing_tools,
        "bcftools_available": bcftools_available,
        "versions": get_tool_versions(backend) if backend else {},
        "portable_bundle": portable,
        "install_mode": "offline-portable",
    }
...
def run():
    ...
    if module == "wgs":
        environment_state = wgs_environment_status()
        if not environment_state["ready"]:
            return (
                jsonify(
                    {
                        "status": "failed",
                        "message": "Portable WGS tools are not installed. Download and install the offline TMM BioSeq WGS tools package.",
