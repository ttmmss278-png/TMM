...
echo [3/5] 正在检测 WSL Ubuntu...
call :detect_distro
if not defined DISTRO (
    echo [错误] 未找到已初始化的 Ubuntu WSL。
    echo 请先在 Windows 中安装并启动一次 Ubuntu，然后重新运行本安装器。
    goto :failed
)
echo 已找到：%DISTRO%
...
wsl.exe -d "%DISTRO%" -u root -- bash -lc "test -x /opt/tmm-bioseq-wgs/bin/fastp && test -x /opt/tmm-bioseq-wgs/bin/bwa && test -x /opt/tmm-bioseq-wgs/bin/samtools && test -x /opt/tmm-bioseq-wgs/bin/bcftools" 1>>"%LOG%" 2>>"%LOG%"
if errorlevel 1 (
    echo [错误] WGS 工具完整性校验失败。
    goto :failed
)
